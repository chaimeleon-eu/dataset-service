...
def getAutoPID(db, dataset):
    datasetId = dataset["id"]
    newValue = db.getZenodoDOI(datasetId)
    if newValue is None: 
        ...
        newValue = ZENODO_PREFIX + newValue
    return newValue
...
def patchDataset(id):
    ...
    with DB(CONFIG.db) as db:
        ...
        if property == "draft":
            ...
            if False and bool(newValue) and dataset["pidUrl"] is None:
                # When publish, a PID will be autogenerated if it is still none 
                pid = getAutoPID(db, dataset)
                db.setDatasetPidUrl(datasetId, str(pid))
            ...
            if str(newValue).startswith(ZENODO_PREFIX):
                newValue = getAutoPID(db, dataset)
